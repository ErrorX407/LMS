  ...
  return (
    <div className="post relative m-auto h-fit xsm:w-[200px] moblieLg:h-[220px] lg:w-[250px] lg:h-[260px] moblieLg:bg-[rgba(255,255,255,0.1)] flex-shrink-0 rounded-xl moblieLg:rounded-2xl overflow-hidden md:w-[220px] md:h-[240px]">
      ...
      ></Link>
        ...
        <Link
          to={`/editor/${postLink}`}
          className="z-[99] absolute top-3 moblieLg:bottom-3 moblieLg:top-1/2 md:top-[55%] lg:top-[60%] right-3 bg-black/30 backdrop-blur-md w-fit flex p-3 rounded-2xl justify-start items-center gap-2 cursor-pointer hover:bg-black/60"
        >
          <img
      ...
      <div className="moblieLg:hidden w-full px-2 py-4 flex justify-between items-center">
          <div>
        ...
        <div className="w-full z-[9] absolute left-1/2 px-3 -translate-x-1/2 bottom-2  flex justify-between items-center h-12">
          <div>
            <div className="flex justify-start items-center gap-2 md:gap-3">
              ...
              <div className="Published-date capitalize text-[13px] moblieLg:text-[12px] md:text-[15px] lg:text-[16px] w-[100%] overflow-hidden whitespace-nowrap text-ellipsis text-white/70">
                {getFullDate(publishedAt)}
              </div>
            </div>
          </div>
          <Link to={`/${category}`}>
            <div className="p-1 xsm:p-1.5 md:p-2 rounded-2xl tags bg-white/10 z-[10] hover:opacity-80">
              <span className="capitalize text-[12px] xsm:text-[13px] md:text-[14px] lg:text-[14px]">{category}</span>
            </div>
          </Link>
        </div>
      </div>
      <Link
        to={`/post/${postLink}`}
        className="absolute top-0 left-0 w-full h-full hidden moblieLg:block"
      ></Link>
    </div>
  );
};
